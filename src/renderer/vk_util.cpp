    ...
    VkMemoryPropertyFlags memProps)
{
    ...
    NB_ASSERT(typeIdx != INVALID_IDX, "Failed to fine proper type Index for Memory Properties: %d", memProps);
...
Buffer vk_allocate_buffer(VkDevice device, VkPhysicalDevice gpu, uint32_t size, VkBufferUsageFlags bufferUsage, VkMemoryPropertyFlags memProps)
{
    Buffer buffer = {};
    buffer.size = size;
...
{
    NB_ASSERT(buffer->size >= size, "Buffer too small: %d for data: %d", buffer->size, size);
    if (buffer->data)
    {
    ...
    {
        // TODO: copy data using command buffer
    }
}